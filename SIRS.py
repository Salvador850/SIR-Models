...
data_file = open("/home/salvadoor/SFI Internship/SIR Models/medium_urbanization_data.txt", 'r').read()
data_lists = data_file.split(']]')

data_lists =  '[' + ']], '.join(data_lists)[:-2].replace('None', '') + ']'
data_lists = eval(data_lists)

data_file = open("/home/salvadoor/SFI Internship/Real World Data Normalization/normalized_data.txt", 'r').read()
...
if eval(sys.argv[9]) == True:
    ...
    new_I = [i / 30 for i in new_I]
    real_data_list = list(data_frame['target'])
    for data_point in range(500):
        mse.append((real_data_list[data_point] - new_I[data_point]) ** 2)
